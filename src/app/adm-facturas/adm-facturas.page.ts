...
import { FacturaService } from '../Servidores/factura.service';
...
@Component({
  ...
  templateUrl: './adm-facturas.page.html',
  styleUrls: ['./adm-facturas.page.scss'],
})
export class AdmFacturasPage implements OnInit {
   list_fact:any

  constructor( private serv:FacturaService) { }

  ngOnInit() {
    this.list_fact=this.serv.lista();
  }

  imprimir(e) { 
    let content = document.getElementById("contenido"+e.id);
    const doc = new jsPDF();
    ...
    // Poner estado de Factura en TRUE 
    }

  borrar(e:Factura) { this.serv.borrar(e); this.ngOnInit(); }
  ...
  NumeroALetras(num, currency) {
        ...
        let data = {
            ...
            letrasMonedaSingular: currency.singular || 'Boliviavo', //'PESO', 'Dólar', 'Bolivar', 'etc'
